...
def proxy_handler(client_socket, remote_host, remote_port, receive_first):
    ...
    if receive_first:
        ...
        if len(remote_buffer):
            logging.info("[<==] Send %d bytes to localhost." % len(remote_buffer))
            client_socket.send(remote_buffer)
    while True:
        local_buffer = receive_from(client_socket)
        if len(local_buffer):
            logging.info("[==>] Rec %d bytes from localhost." % len(local_buffer))
...
def server_loop(local_host, local_port, remote_host, remote_port, receive_first):
    ...
    try:
        ...
        logging.warning("[!!] Check for other listening sockets or correct permissions.")
        sys.exit(0)
    ...
    while True:
        ...
        logging.info("[==>] Received incoming connection from %s:%d" %
              (addr[0], addr[1]))
